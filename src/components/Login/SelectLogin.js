...
class SelectLogin extends Component {
  ...
  handleOnChange = e => {
    ...
    if (state && state.from && state.details && isValidPoll()) {
      ...
      // TODO: error handle undefineed
      const selQuestion = this.props.questions.filter(f => f.id === state.details.params.question_id).pop()
  ...
  render () {
    // console.log('SelectLogin:location:state', this.props.location.state);
