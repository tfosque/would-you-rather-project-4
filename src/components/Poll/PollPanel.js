...
class PollPanel extends Component {
  ...
  componentDidMount () {
    ...
    /*  if (this.props.category === 'answered') {
       this.props.showPollDetails(true)
     } */
  }

  findUsersAnswers = (user) => {
  ...
  render () {
    ...
    // FIX: fix default styles
    let defaultStyles;
    const useRedirectOption = typeof redirectOption !== 'undefined' ? redirectOption.option : {} // sessionStorage.getItem('myRedirect')
    ...
    if (selectedPollOption === pollQuestion.option && showPollDetails && (selectedPollOption === 'optionOne' || selectedPollOption === 'optionTwo')) {
      // console.log('if: 04');
      defaultStyles = `${ selectedPollOption } selected`
    } else if ((selectedPollOption !== 'optionOne' && selectedPollOption !== 'optionTwo') && showPollDetails) {

      defaultStyles = pollQuestion.option === useRedirectOption ? `${ useRedirectOption } selected` : 'omitSelection'
    } else {
      defaultStyles = 'omitSelection'
    }
        ...
        >
          { showPollDetails ? (
            <PollDetails
              ...
              selectedPollOption={ selectedPollOption } // FIX
